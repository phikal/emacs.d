...
*** RFC
	#+BEGIN_SRC emacs-lisp
	  (when (file-directory-p "/usr/share/doc/RFC/links/")
...
*** Timestamps
	...
	If files have timestamps, make sure that they are updated every time
	a buffer is saved.

	This will search the first few lines of a file for a line that
	contains like one of the following:
	...
	and insert a timestamp + username.
...
** Programming
   ...
   Most programming languages are whitespace insignificant, and if they
   are not, they /usually/ don't care about the whitespaces at the end
   of a line. For that reason, Emacs shall make sure that these are
   removed before saving a file.
...
** Writing
   ...
   Just as with programming modes above, trailing whitespaces have no
   need to exist in text files. They should be deleted before saving a
   file.
