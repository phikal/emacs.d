...
*** User information
	...
	#+BEGIN_SRC emacs-lisp
	  (setq user-full-name "Philip K."
			user-mail-address
			(format "%s%c%s.net" "philip" (- ?A 1) "warpmail"))
	#+END_SRC
...
***** Project Managment
	  :PROPERTIES:
	  :WEBSITE:  websihttps://github.com/bbatsov/projectil
...
***** Jump in Buffer
	  ...
	  Avy is an alternative to [[https://www.emacswiki.org/emacs/AceJump][ace-jump]], which itself is supposed to be
...
***** Auto-Pairing Parentheses
	  ...
	  #+BEGIN_SRC emacs-lisp
		(use-package elec-pair
		  :hook ((minibuffer-setup-hook electric-pair-local-mode)
				 (text-mode-hook electric-pair-local-mode)
				 (prog-mode-hook electric-pair-local-mode)
				 (comint-mode-hook electric-pair-local-mode)))
		  :demand t
	  #+END_SRC
...
***** TODO Rmail
	  #+BEGIN_SRC emacs-lisp
		(use-package rmail
		  :if (and (executable-find "movemail")
				   (executable-find "mpop"))
		  :custom ((rmail-file-name "~/etc/mail/rmail")
...
**** IRC
	 ...
	 [[*Local or Private Configurations][=custom.el=]] file.
