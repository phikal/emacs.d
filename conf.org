...
*** Automatically create new lines
	...
	When wanting to move beyond the end of the buffer, setting this
	variable lets the buffer automatically grow instead of just saying
	to the cursor.
	  ...
	  #+BEGIN_SRC emacs-lisp
			   ...
			   (defun zge/format-mail (name)
				 (let* ((dir-name (concat "~/etc/mail/" name "/new"))
						...
						(count (length dir))
						(from-list (and (<= 1 count 3)
										(mapconcat #'zge/mail-from dir ", "))))
				   (cond ((= count 0) nil)
						 (from-list (format "in %s: %d (%s)" name count from-list))
						 (t (format "in %s: %d" name count)))))
			   ...
			   (defun zge/fetch-mail ()
				 ...
				 (async-start (lambda ()
							  ...
							  (lambda (&rest _)
								(let* ((mail (seq-filter #'identity
														 (mapcar #'zge/format-mail
																 (list "uni" "pers"))))
									   (new-msgs (mapconcat #'identity mail ", ")))
								  (unless (string-empty-p new-msgs)
									(message "new mail: %s" new-msgs))))))
...
**** LaTeX
	 #+BEGIN_SRC emacs-lisp
	   (use-package auctex
		 ...
		 :config
		 (setf LaTeX-electric-left-right-brace t
			   (alist-get 'output-pdf TeX-view-program-selection) '("PDF Tools")
			   TeX-master 'dwim
			   TeX-auto-save t
			   TeX-parse-self t
...
**** Snippets
	 #+BEGIN_SRC emacs-lisp
	   (use-package yasnippet
		 ...
		 :functions (yas-expand indent-then-expand)
		 :config
