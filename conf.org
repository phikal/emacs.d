...
*** Set value locally
	#+BEGIN_SRC emacs-lisp
	  (defmacro locally-setq (sym val)
		"Produce a lambda expression that locally sets a value"
		`(lambda () (setq-local ,sym ,val)))
	#+END_SRC
	   ...
	   (use-package rcirc
		 ...
		 (setq rcirc-server-alist
			   `(("zge.us.to"
				  ...
				  :port 23551
				  :encryption tls))
			   rcirc-reconnect-delay 8
			   rcirc-read-only-flag t
			   rcirc-log-flag t
			   rcirc-omit-responses '("NICK" "AWAY"))
		 (add-hook 'rcirc-mode-hook #'flyspell-mode)
		 (add-hook 'rcirc-mode-hook #'electric-pair-local-mode)
...
**** LaTeX
	 #+BEGIN_SRC emacs-lisp
	   (use-package auctex
		 :config
		 (progn
		   (add-hook 'LaTeX-mode-hook (locally-setq company-backends
													'(company-latex-commands
													  company-math)))
		   (dolist (mode '(flycheck-mode reftex-mode TeX-fold-mode))
	 ...
	 #+BEGIN_SRC emacs-lisp
	   (use-package go-mode
		 ...
		 (add-hook 'go-mode-hook
				   (locally-setq compile-command "go generate && go build -v && go test -v && go vet"))
		 (add-hook 'go-mode-hook
	   ...
	   (use-package company-go
		 ...
		 :init
		 (add-hook 'go-mode-hook (locally-setq company-backends '(company-go))))
	 ...
	 #+BEGIN_SRC emacs-lisp
	   ...
	   (use-package slime-company
		 ...
		 (add-hook 'slime-mode-hook
				   (locally-setq company-backends '(company-slime))))
	 #+END_SRC
	 ...
	 #+BEGIN_SRC emacs-lisp
	   (use-package company-ghc
		 ...
		 (add-hook 'haskell-mode-hook
				   (locally-setq company-backends '(company-ghc)))
		 :after company)
	 #+END_SRC
	   ...
	   (unless (boundp 'zge/dict-ring)
		 (setq-local zge/dict-ring
					 (let ((ring (make-ring (length zge/dicts))))
					   (mapc (apply-partially #'ring-insert ring)
							 zge/dicts)
					   ring)))
