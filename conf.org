...
*** Images
	...
	For some reason, these two functions are not bound by default in
	=image-mode=, but since they are quite useful, I do so instead.
...
**** IRC
	 #+BEGIN_SRC emacs-lisp
	   (use-package rcirc
		 ...
		 :custom ((rcirc-server-alist `(("zge.us.to"
										 :user-name "zge"
										 :nick "zge"
