...
*** Form feed as line
	#+BEGIN_SRC emacs-lisp
      (aset (or standard-display-table
            ...
            ?\^L (make-vector 64 (make-glyph-code ?― 'font-lock-constant-face)))
	#+END_SRC

	Usually form-feed charachters (ASCII 12), are reprsented by a simple
	=^L=, and are used to designate sections. This little hack replaces it
	with a unicode line (consisting of 64 "HORIZONTAL BAR" unicode
	charachters).
...
*** Recent files
	...
	For tools like [[*Project%20Managment][Project Managment]], recentf is enabled, so to easily access
...
***** Gnus
	  ...
	  #+BEGIN_SRC emacs-lisp
        (setq gnus-select-method '(nnmaildir "mail" (directory "~/etc/mail/")))
	  #+END_SRC

	  I have two accounts I use (personal and university Email), and bot
	  are synchronized via [[https://marlam.de/mpop/][mpop]], my [[https://zge.us.to/files/authinfo][authinfo]] script and the following
	  configuration:
...
***** SMTP
	  ...
	  As already mentioned, this requires a [[https://zge.us.to/files/authinfo][special script]] I wrote to
   ...
   #+BEGIN_SRC emacs-lisp
     (defun zge/org-check-lang ()
       ...
       (save-excursion
         (goto-char (point-min))
         (catch 'found
           (while (not (looking-at-p (rx (or (group bol (zero-or-more space) eol)
                                             (group (zero-or-more not-newline) eos)))))
             (save-match-data
               (when (looking-at (rx bol "#+LANGUAGE: " (group (one-or-more word))))
                 (let ((lang (match-string-no-properties 1)))
                   (zge/toggle-dictionary (pcase lang
                                            ("en" "british")
                                            ("de" "german-new8")
                                            (t lang)))
          ...
          (while (< (point) until)
            (save-match-data
              (when (looking-at (rx "#include <"
                                    ...
                                    (group (minimal-match (one-or-more nonl)))
