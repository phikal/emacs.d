...
*** Auto-Pairing Parentheses
	...
	While I usually am not too much of a fan of global minor modes, I
	use =electric-pair-mode= practically everywhere, and miss it if it
	it's not found. The main advantage as compared to manually using
	=M-(=, asides from not having to manually use the meta key, is that it
	also removed adjacent matching parentheses with just one keystroke
	-- as soon as you get used to this, you'll miss it everywhere.
