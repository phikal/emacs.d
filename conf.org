...
***** Gnus
	  ...
	  #+BEGIN_SRC emacs-lisp
		(defun zge/fetch-mail ()
		  (interactive)
		  (async-start-process "*mpop*" (executable-find "mpop") #'ignore "-aQ"))
...
**** C
	 ...
	 #+BEGIN_SRC emacs-lisp
	   (use-package irony
		 ...
		 (condition-case err (irony--find-server-executable)
		   (error (call-interactively #'irony-install-server)))
		 :mode (rx ".c" eos))
	 ...
	 Irony is integrates libclang into Emacs, making eldoc, error
	 checking and completion work faster and more accurately than with
	 the default setup. This requires installing a backend server.
