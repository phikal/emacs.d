...
** Insert C headers
   #+BEGIN_SRC emacs-lisp
     (defconst zge/c-headers-manual
       '("stdbool.h"))

     (defvar zge/c-headers nil)
     (defun zge/c-list-headers ()
       (or zge/c-headers
           (let ((hdrs (mapcar (lambda (header) (string-remove-prefix "/usr/include/" header))
                               (directory-files-recursively "/usr/include/" "\\.h$"))))
             (setq zge/c-headers (append zge/c-headers-manual hdrs)))))
     ...
     (defun zge/c-insert-header (header)
       ...
       (save-mark-and-excursion
        ...
        (sort-lines nil (point-min) (point))))
