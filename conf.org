...
*** Fonts
	#+BEGIN_SRC emacs-lisp
      (set-face-font 'default "Input Mono-12")
      (set-face-font 'variable-pitch "Input Sans-12")
	#+END_SRC
...
**** Auto-completion
	 ...
	 to further integrage Ivy features into default commands, such as =M-x=,
	 =C-x b= or =C-x C-f=.
...
**** Window Managment
	 #+BEGIN_SRC emacs-lisp
       (use-package winner
         ...
         :config
         (winner-mode)
         :bind (("C-c <next>" . winner-undo)
                ("C-c <prior>" . winner-redo)))
	 #+END_SRC
	 ...
	 since this conflics with my muscle memory for flycheck's
	 ...
	 #+BEGIN_SRC emacs-lisp
       (use-package helpful
         ...
         :bind (("C-h f" . helpful-function)
                ("C-h k" . helpful-key)
                ("C-h v" . helpful-symbol)))
	 #+END_SRC

	 One of Emacs' greatest virtues is it's built in (or "self
	 documenting") documentation system that helps the user explore what
	 one can do and customize. This package extends/replaces the default
	 UI making it more pleasant to work with and look at.
...
**** IRC
	 #+BEGIN_SRC emacs-lisp
       (use-package rcirc
         ...
         (define-key rcirc-mode-map (kbd "C-x k") 'bury-buffer)
         :config
         (rcirc nil)
         (rcirc-track-minor-mode t))
	 #+END_SRC

	 For IRC, I use Rcirc and connect to my bouncer server (currently
	 ZNC). The =zge/znc-pass= variables is declared in my [[*Emacs-generated%20files][custom.el]]
	 file. On starting my Emacs session, rcirc automatically connects
	 ...
	 about my notes and agenda file.
	  ...
	  #+BEGIN_SRC emacs-lisp
        (setq org-export-date-timestamp-format "%x"
              org-html-head "<style>body{max-width:50em;text-align:justify;margin:2% auto;}</style>"
