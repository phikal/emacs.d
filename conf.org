...
***** Gnus
	  ...
	  #+BEGIN_SRC emacs-lisp
		(defun zge/compose-mail (arg)
		  "Compose a mail with gnus posting style."
		  (interactive "P")
		  (let* ((groups '("pers" "uni"))
				 (gnus-newsgroup-name
				  (if arg
					  (completing-read "Group to use: " groups)
					(car groups))))
			(gnus-setup-message 'message (message-mail))))
	  #+END_SRC

	  Since =compose-mail=, which is by default bound to =C-x m= /doesn't/
	  insert my posting styles/signatures, this function wraps gnus
	  setup routine to construct a buffer.
...
**** LaTeX
	 #+BEGIN_SRC emacs-lisp
	   (use-package auctex
		 ...
		 (add-hook 'LaTeX-mode-hook (setl company-backends '(company-latex-commands)))
   ...
   #+BEGIN_SRC emacs-lisp
	 (defun zge/org-check-lang ()
	   ...
	   (save-excursion
		 (goto-char (point-min))
		 (catch 'found
		   (while (not (looking-at-p (rx (seq bol (* space) eol))))
			 (save-match-data
