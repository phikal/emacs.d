...
*** Fonts
	#+BEGIN_SRC emacs-lisp
	  (set-face-font 'default "APL385 Unicode 13")
	  (set-face-font 'variable-pitch "Sans")
	  (set-face-font 'fixed-pitch "Monospace")
	#+END_SRC
   ...
   #+BEGIN_SRC emacs-lisp
	 (defvar zge/registers
	   `((?~ "~")
		 ...
		 (?p "~/doc/org/pers.org")
		 (?j ,(with-temp-buffer
				(insert-file-contents "~/doc/org/.org-agenda")
				(goto-char (point-min))
				(expand-file-name
...
*** Configure /Completion At Point/
	#+BEGIN_SRC emacs-lisp
	  (defmacro capf-conf (hook func)
		"Configure a `completion-at-point' backend for a mode, by
	  adding a function to locally append `func' to
	  `completion-at-point-functions'."
		`(add-hook (quote hook)
				   (setl completion-at-point-functions
						 (cons ,func completion-at-point-functions))))
	#+END_SRC
...
**** C
	 ...
	 #+BEGIN_SRC emacs-lisp
	   (use-package cpp-capf
		 ...
		 :load-path "lisp/"
		 :init
		 :config (capf-conf c-mode-hook #'cpp-completion-at-point-function))
	 #+END_SRC
...
**** Go
	 ...
	 #+BEGIN_SRC emacs-lisp
	   (use-package go-capf
		 ...
		 :config
		 (require 'go-capf)
		 (capf-conf go-mode-hook #'go-completion-at-point-function))
	 #+END_SRC
...
** Timestamp
   #+BEGIN_SRC emacs-lisp
	 (defun zge/add-timestamp (keep)
	   "Prepend a special UNIX timestamp to buffer"
	   (interactive "P")
	   (save-excursion
		 (goto-char (point-min))
		 (insert (format-time-string
				  (if (or keep (looking-at-p "<!--@ [[:digit:]]+ "))
					  (progn
						(kill-word 1)
						"<!--@ %s")
					"<!--@ %s -->\n\n")))))
